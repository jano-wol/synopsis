<template>
    <h2 class="fs-3">
        E-mail
    </h2>
    <p>Please send any comments, questions or suggestions to:</p>
    <p><b>contact@synopticus.org</b></p>
    <h2 class="fs-3">
        Wiki
    </h2>
    <p>
        The project includes a <a href="https://github.com/anwolosz/synopsis/wiki">Wiki</a> page that documents decisions, questions, and observations made during development. The errata section contains errors identified by users.
    </p>
    <h2 class="fs-3">
        Development
    </h2>
    <p><a href="https://www.synopticus.org/">synopticus.org</a> is an open source project. It is developed on GitHub. The repository is available at
        <a href="https://github.com/anwolosz/synopsis" target="_blank">this</a> link.
    </p>
    <p>Currently, the aim is to integrate the different Gospel translations. The processed translations (in <code
            class="text-dark">.json</code> format) can be found in the <router-link :to="{name: 'sources'}">Sources</router-link> page, under Downloadable Materials.
    </p>
</template>